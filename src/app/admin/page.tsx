...
export default function DashboardPage() {
  ...
  const [websiteLeads, setWebsiteLeads] = useState<WebsiteLeadSummary | null>(null);
  ...
  useEffect(() => {
    ...
    api
      .get<WebsiteLeadSummary>("/api/website-leads/summary")
      .then(setWebsiteLeads)
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
        ...
        <StatTile label="Today's Follow-ups" value={data.followUps.todayCount} href="/admin/follow-ups#today" />
        <StatTile label="Upcoming Follow-ups" value={data.followUps.upcomingCount} href="/admin/follow-ups#upcoming" />
      </div>
      ...
      <Card className="mt-6">
        <CardHeader
          title="Website Lead Overview"
          subtitle="Public-site enquiries — Google Sheets stays the primary record."
          action={<LinkButton href="/admin/website-leads" variant="ghost" size="sm">View all →</LinkButton>}
        />
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3 sm:gap-4 p-4 sm:p-5">
          <StatTile label="New Leads" value={websiteLeads?.newLeads ?? 0} hint={websiteLeads ? `${websiteLeads.todayLeads} today` : undefined} />
          <StatTile label="Hot Leads" value={websiteLeads?.hotLeads ?? 0} />
          <StatTile label="Follow Ups" value={websiteLeads?.followUps ?? 0} />
          <StatTile label="Meetings" value={websiteLeads?.meetings ?? 0} />
          <StatTile label="Proposals" value={websiteLeads?.proposals ?? 0} />
          <StatTile label="Won" value={websiteLeads?.won ?? 0} />
        </div>
      </Card>

      <Card className="mt-6">
        <CardHeader title="Recent Leads" action={<LinkButton href="/admin/leads" variant="ghost" size="sm">View all →</LinkButton>} />
