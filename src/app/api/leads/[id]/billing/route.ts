...
export async function GET(_req: NextRequest, ctx: RouteContext<"/api/leads/[id]/billing">) {
  try {
    ...
    return apiErrorResponse(err, "GET /api/leads/[id]/billing");
  }
}
...
export async function PATCH(req: NextRequest, ctx: RouteContext<"/api/leads/[id]/billing">) {
  try {
    ...
    if ("advanceAmount" in body)
      patch.advanceAmount = body.advanceAmount === "" || body.advanceAmount == null ? null : Number(body.advanceAmount);
    if ("monthlyFeeAmount" in body)
      patch.monthlyFeeAmount = body.monthlyFeeAmount === "" || body.monthlyFeeAmount == null ? null : Number(body.monthlyFeeAmount);
    if ("projectFeeAmount" in body)
      patch.projectFeeAmount = body.projectFeeAmount === "" || body.projectFeeAmount == null ? null : Number(body.projectFeeAmount);
    if ("currency" in body) patch.currency = body.currency || null;
    if ("billingNotes" in body) patch.billingNotes = body.billingNotes || null;
    if ("taxExempt" in body) patch.taxExempt = Boolean(body.taxExempt);
    ...
    return apiErrorResponse(err, "PATCH /api/leads/[id]/billing");
  }
}
